...
export default function UsersTable({ currentUserId, canDeleteOthers }: UsersTableProps) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      // Se o usuário se auto-excluiu, recarrega a página para deslogar
              ...
              <TableBody>
                {users.map((user) => (
                  <TableRow key={user.id} data-state={selectedUserIds.includes(user.id) ? "selected" : ""}>
                    ...
                    <TableCell>
                      <div className="font-medium">{user.nome || 'N/A'} {user.sobrenome}</div>
                    ...
                    <TableCell>
                      {(canDeleteOthers || user.id === currentUserId) && (
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => {
